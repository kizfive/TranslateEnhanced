...
class Translate : Plugin() {
    ...
    override fun load(ctx: Context) {
        ...
        controller = TranslateController(
            ...
            isChannelAutoEnabled = { channelId -> autoManager.isEnabled(channelId) }
        )
    ...
    private fun maybeAutoTranslate(channelId: Long, messageId: Long) {
        ...
        controller.submit {
            ...
            try {
                ...
                // 持久缓存命中：直接显示译文，不请求后端
                val cached = TranslationCache.get(messageId, safeContent)
                if (cached != null) {
                    DebugLogger.log("auto cache hit: msg=$messageId")
                    controller.cacheResult(
                        messageId,
                        TranslateResult.Success(
                            sourceLanguage = cached.sourceLanguage,
                            translatedLanguage = cached.translatedLanguage,
                            sourceText = safeContent,
                ...
                if (channelId != 0L) {
                    ...
                    linearLayout.addView(TextView(ctx, null, 0, R.i.UiKit_Settings_Item_Icon).apply {
                        ...
                        setOnClickListener {
                            ...
                            // 关闭时：清掉该频道排队中的翻译，正在进行的响应到达后也会被丢弃
                            if (!nowOn) controller.cancelAutoChannel(channelId)
                            Utils.showToast(
                    ...
                    // 频道翻译配置（专属提示词 + 术语表 + 缓存管理）
                    linearLayout.addView(TextView(ctx, null, 0, R.i.UiKit_Settings_Item_Icon).apply {
                        text = strings.actionChannelConfig
                        setCompoundDrawablesRelativeWithIntrinsicBounds(pluginIcon, null, null, null)
                        setOnClickListener {
                            showChannelConfigDialog(ctx, channelId)
                            (hookParam.thisObject as WidgetChatListActions).dismiss()
                        }
                    })
                }
    ...
    /** 频道翻译配置对话框：编辑提示词/术语表、自动生成、清除配置/缓存。 */
    private fun showChannelConfigDialog(ctx: Context, channelId: Long) {
        ...
        MaterialAlertDialogBuilder(ctx)
            .setTitle(strings.channelConfigTitle)
            .setView(layout)
            ...
            .setNeutralButton(strings.channelConfigGenerate) { _, _ ->
                generateChannelConfig(ctx, channelId, promptInput, glossaryInput)
            }
            .show()
    }

    /** 用大模型分析频道最近消息，生成提示词 + 术语表初稿（填充到对话框，用户确认后保存）。 */
    private fun generateChannelConfig(
        ctx: Context,
        channelId: Long,
